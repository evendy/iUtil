...
public class BaseDialog extends Dialog implements
	...
	public void onClick(View v) {
		/**
		 * 当我们在Android依赖库中使用switch-case语句访问资源ID时会报如下图所示的错误，报的错误是case分支后面
		 * 跟的参数必须是常数，换句话说出现这个问题的原因是Android library中生成的R.java中的资源ID不是常数
		 * (由于本包声明了是lib包)
		 */
		if (v.getId() == R.id.dialog_generic_btn_button1) {
			if (mOnClickListener1 != null) {
				mOnClickListener1.onClick(mBaseDialog, 0);
			}
		} else if (v.getId() == R.id.dialog_generic_btn_button2) {
			if (mOnClickListener2 != null) {
				mOnClickListener2.onClick(mBaseDialog, 1);
			}
		} else if (v.getId() == R.id.dialog_generic_btn_button3) {
			if (mOnClickListener2 != null) {
				mOnClickListener2.onClick(mBaseDialog, 1);
			}
		}
