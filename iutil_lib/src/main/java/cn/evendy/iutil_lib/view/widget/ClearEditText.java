	...
	public boolean onTouchEvent(MotionEvent event) {
		if (getCompoundDrawables()[2] != null) {
			if (event.getAction() == MotionEvent.ACTION_UP) {
				// 当点击此控件的位置在此图片的左边缘与右边缘的时候就将控件中的内容删除
				int mLeft = getWidth() - getPaddingRight()
						- mClearDrawable.getIntrinsicWidth();
				int mRight = getWidth() - getPaddingRight();
				boolean touchable = event.getX() > mLeft
						&& (event.getX() < mRight);
				if (touchable) {
					this.setText("");
				}
